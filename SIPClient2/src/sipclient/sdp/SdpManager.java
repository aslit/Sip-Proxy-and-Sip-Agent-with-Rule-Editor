...
public class SdpManager {
  ...
  public byte[] createSdp(SdpInfo sdpinfo) {
    ...
    BandWidth myBandWidth = mySdpFactory.createBandwidth(BandWidth.AS, sdpinfo.band);
    Vector myBandwithVector=new Vector();
    myBandwithVector.add(myBandWidth);
    ...
    mySdp.setBandwidths(myBandwithVector);
  ...
  public SdpInfo getSdp(byte[] content) {
    ...
    try{
      ...
      Vector myBandwithVector=recSdp.getBandwidths(false);
      BandWidth BandWidth = (BandWidth) myBandwithVector.elementAt(0);
      int myBandWidth = BandWidth.getValue();
     
   
      
      
      String myPeerIp=recSdp.getConnection().getAddress();
     /* Vector BandWidth = recSdp.getBandwidths(false);
      int myBandWidth = Integer.parseInt(BandWidth.elementAt(0).toString());
 */
